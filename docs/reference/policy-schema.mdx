...
folded only when Windows confirms that every parent directory of the
runtime-observed executable uses case-insensitive lookup. If any component is
case-sensitive or cannot be checked, matching remains case-exact and fails
closed. Loading policy never opens a policy-authored path. UNC, device, and
verbatim namespace paths retain their exact spelling and separators.
