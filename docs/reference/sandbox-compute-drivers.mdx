...
ignores them. The MXC driver rejects CPU and memory requests because its current
backends cannot enforce them.

API clients that populate `SandboxTemplate.resources` directly must encode the
`cpu` and `memory` values under `limits` or `requests` as non-empty strings.
The gateway rejects malformed sections and non-string values with
`InvalidArgument` instead of forwarding them as opaque driver configuration.
